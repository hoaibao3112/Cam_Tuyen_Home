...
interface ProductFormModalProps {
  ...
  onRenameSubCategory: (category: string, oldSubCategory: string, newSubCategory: string) => Promise<void>
}
                            ...
                            <div className="flex items-center gap-1 flex-shrink-0">
                              ...
                              <button
                                ...
                                onClick={e => {
                                  e.stopPropagation()
                                  setIsSubDropdownOpen(false)
                                  const newName = prompt('Nhập tên nhóm phụ mới:', sub)
                                  if (newName === null) return
                                  const trimmed = newName.trim()
                                  if (!trimmed || trimmed === sub) return
                                  onRenameSubCategory(form.category, sub, trimmed)
                                }}
                                className="size-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors cursor-pointer"
                                title={`Đổi tên nhóm phụ "${sub}"`}
                              >
                              ...
                              <button
                                ...
                                className="size-7 flex items-center justify-center rounded-lg text-red-400 hover:text-red-600 hover:bg-red-50 transition-colors flex-shrink-0 cursor-pointer"
